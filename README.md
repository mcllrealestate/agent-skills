...
- **MCP server** — `https://mcllrealestate.com/api/mcp` (Streamable HTTP). Tools: `search_listings` (city, area, type, bedrooms, price), `get_listing` (full detail), and `execute` (Code Mode JavaScript sandbox over `mcll.search` / `mcll.get`). The richest path; it resolves human names ("condos in Phuket") to filters and can compare, rank, or compute across many listings in one call.
